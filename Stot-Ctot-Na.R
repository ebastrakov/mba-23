# A script to build a Stot-Ctot diagram for uranyl complexes paper
...
# bases <- c("SO4-2", "HSO4-", "S")
## Use this to change both C and S species
bases <- list(c("CO2", "HCO3-"), c("SO4-2", "HSO4-", "S"))

args <- list(bases, c(c(-4,0.5), 600), c(c(-4,0.5), 600), T, P, IS)
names(args) <- c("bases", "SO4-2", "CO2", "T", "P", "IS")
...
# diagram(a$A.bases[[1]], add=TRUE, fill = NA, lty=2, lwd=1, col="red", col.names="red", cex.names=1)
# diagram(a$A.bases[[2]], add=TRUE, fill = NA, lty=2, lwd=1, col="#3C8DBC", col.names="#3C8DBC", cex.names=1)
...
dTP <- describe.property(c("T", "P", "pH"), c(T, P, pH))
